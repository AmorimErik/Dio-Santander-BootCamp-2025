...
@router.get('/{id}', summary='Consultar uma Categoria pelo id', status_code=status.HTTP_201_OK, response_model=CategoriaOut)
...
async def query(id: UUID4, db_session: DatabaseDependency) -> CategoriaOut:
    ...
    if not categoria:
        raise HTTPExcepion(status_code=status.HTTP_404_NOT_FOUND, detail='Categoria não encontrada no id: {id}')
    return categoria